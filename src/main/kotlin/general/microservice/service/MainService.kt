...
class MainService {
    ...
    private val url = URL("https://www.cbr.ru/scripts/XML_dynamic.asp?date_req1=23/04/2022&date_req2=23/04/2022&VAL_NM_RQ=R01235")
    ...
    fun getValue(): String {
        ...
        val test = BufferedReader(InputStreamReader(bis))
            .lines().collect(Collectors.joining("\n"))
        val serializer: Serializer = Persister()
        val valCurs = serializer.read(ValCurs::class.java, test)
        val value = valCurs.list[valCurs.list.size-1].value!!.replace(',', '.')
        val mainEntity = MainEntity(value)
        repository.save(mainEntity)
        bis.close()
        logger.info { "Request successful done" }
        return if (value.toDouble() < giveValue) {
